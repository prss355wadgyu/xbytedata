...
# res=requests.get(url='https://fddb.info/db/de/lebensmittel/akakiko_sushi_set_mini/index.html')
# b=res.text
# response=Selector(text=b)
# for i in response.xpath('//div[@class="sidrow"]/following-sibling::div'):
...
#
#     #
#     # # for i in response.xpath('//div[@class="standardcontent"]'):
#     #
#     #
#     # for i in response.xpath('//div[@class="sidrow"]/following-sibling::div'):
#     #     sub_cat=i.xpath('.//text()').get()
...
def get_data(sub_item_link,row):
 ...
 + '`')
        ...
        try:
            ...
            try:
                con = db.cursor()
                update_query = f'UPDATE sdb3.my_table4 SET status = "completed" WHERE sub_item = "{sub_item}"'
