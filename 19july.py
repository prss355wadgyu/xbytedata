...
class NoonMobiles:
    def __init__(self):
        c=db.cursor()
        self.table_name='sport_information'
        self.columns={
    ...
    def scrap(self, url, headers):
        ...
        for item in hits:
            ...
            try:
                query=f"""INSERT INTO db3.sport_information ({key_str}) VALUES ({value_str})"""
                c.execute(query)
                db.commit()
...
if __name__ == '__main__':
    n=NoonMobiles()
    url=f'https://www.noon.com/_svc/catalog/api/v3/u/sports_outdoor-bestseller-AE/?f%5BisCarousel%5D%5B%5D=true&limit=50&page=1&searchDebug=false&sort%5Bby%5D=popularity&sort%5Bdir%5D=desc'
