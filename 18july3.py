...
class Noon:
    ...
    def __init__(self):
        c = db.cursor()
        self.table_name = 'mobile_product_details'
        self.columns = {
    ...
    def scrap(self, link, row):
        ...
        while retry_count < 5:  # Retry up to 5 times
            ...
            try:
                ...
                try:
                    con = db.cursor()
                    query = f"""INSERT INTO db3.mobile_product_details ({i_key}) VALUES ({i_value})"""
                    print(query)
                    con.execute(query)
                    db.commit()
                    ...
                    try:
                        con = db.cursor()
                        update_query = f'UPDATE db3.mobile_information SET status = "completed" WHERE product_name = "{product_name}"'
